...
void main()
{
    ...
    vec4 color_axis = vec4(0.6, 0.6, 0.6, 1.0);         // Color of the axis
    ...
    }else if (abs(world_position.x) < cote && abs(world_position.y) < width) {
        FragColor = color_axis;
    }else if (abs(world_position.y) < cote && abs(world_position.x) < width) {
        FragColor = color_axis;
    }
}