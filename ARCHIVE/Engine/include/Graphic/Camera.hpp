...
class Camera2D {
public:
    Camera2D(float left, float right, float bottom, float top, mat::Vec3f position = mat::Vec3f({0.0f, 0.0f, 0.0f}), float rotation = 0);

    void set_position(mat::Vec3f position);

    void move_position(mat::Vec3f displacement);
    ...
    mat::Mat4f get_view_projection() const;

private:
    mat::Mat4f m_projection;
    mat::Mat4f m_view;
    mat::Mat4f m_view_projection;
